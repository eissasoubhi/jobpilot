...
import styles from './OfferApplicationSummary.module.css';
...
export function OfferApplicationSummary({ application, onApplicationUpdated }: OfferApplicationSummaryProps) {
  const [currentApplication, setCurrentApplication] = useState(application);
  const [reviewOpen, setReviewOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [notice, setNotice] = useState('');
  const [error, setError] = useState('');
  const [undoAvailable, setUndoAvailable] = useState(() => supportsSafeUndo(application));
  ...
  useEffect(() => {
    ...
    setUndoAvailable(supportsSafeUndo(application));
  }, [application]);

  const saveApplication = async (status: string, successMessage: string, offerUndo = false): Promise<void> => {
    if (saving) return;
    setSaving(true);
    setNotice('');
    setError('');
    try {
      ...
      setUndoAvailable(offerUndo && supportsSafeUndo(updated));
  ...
  const savePreparation = async (): Promise<void> => saveApplication(currentApplication.status, 'Modifications enregistrées dans JobPilot.');
  const saveTrackingStatus = async (): Promise<void> => saveApplication(currentApplication.status, 'Statut de suivi enregistré dans JobPilot.');
  const markSubmitted = async (): Promise<void> => {
    if (currentApplication.status !== 'SUBMITTED') await saveApplication('SUBMITTED', 'Candidature marquée comme envoyée. La date d’envoi a été enregistrée dans JobPilot.', true);
  };
  ...
  const undoDecision = async (): Promise<void> => {
    ...
    setError('');
    try {
      const updated = await api<Application>(`/applications/${currentApplication.id}/review-decision/undo`, { method: 'POST' });
      setCurrentApplication(updated);
      onApplicationUpdated?.(updated);
  ...
  return (
    <section className={`notice ${styles.summary}`} aria-label={`Candidature préparée pour ${currentApplication.jobOffer.title}`}>
      <div className={`actions ${styles.summaryHeader}`}>
      ...
      <div className={`actions ${styles.readyBadges}`}>
      ...
      <div className={`actions ${styles.summaryActions}`}>
        <Button variant="secondary" size="small" onClick={() => setReviewOpen(true)}>Examiner</Button>
        {currentApplication.jobOffer.sourceUrl && <a className="btn small" href={currentApplication.jobOffer.sourceUrl} target="_blank" rel="noreferrer">Ouvrir la plateforme pour postuler</a>}
      </div>
      <details className={styles.details}>
        ...
        <div className={`stack ${styles.previewStack}`}>
          {currentApplication.cvDocument && <div><div className="small muted">CV sélectionné</div><div className={`actions ${styles.previewMeta}`}><strong className="small">{currentApplication.cvDocument.name}</strong><a className="btn secondary small" href={currentApplication.cvDocument.downloadUrl} target="_blank" rel="noreferrer">Ouvrir le CV</a></div></div>}
      ...
      {reviewOpen && <Modal ariaLabelledBy={`offer-review-title-${currentApplication.id}`} backdropClassName={styles.reviewBackdrop} panelClassName={styles.reviewDrawer} onClose={() => setReviewOpen(false)}>
        <div className={`actions ${styles.reviewHeader}`}>
          <div><div className="small muted">Examen de l’offre</div><h2 id={`offer-review-title-${currentApplication.id}`} className={styles.reviewTitle}>{currentApplication.jobOffer.title}</h2><div className="small muted">{currentApplication.jobOffer.company || 'Entreprise non renseignée'} · {currentApplication.jobOffer.location || 'Lieu non renseigné'} · {currentApplication.jobOffer.workMode || 'Mode non renseigné'}</div></div>
        ...
        <div className={`stack ${styles.reviewStack}`}>
          ...
          <section><strong>Pourquoi ce score ?</strong>{currentApplication.jobOffer.scoreReasons.length > 0 ? <ul className={styles.scoreReasons}>{currentApplication.jobOffer.scoreReasons.map((reason) => <li className="small" key={reason}>{reason}</li>)}</ul> : <div className={`small muted ${styles.emptyReason}`}>Aucune explication détaillée disponible.</div>}</section>
