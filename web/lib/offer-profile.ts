...
type ContractKind = 'cdi' | 'cdd' | 'freelance' | 'portage' | 'subcontract' | 'other';
...
export function contractKind(value: string): ContractKind {
  ...
  if (/portage/.test(normalized)) return 'portage';
  if (/sous traitance|subcontract/.test(normalized)) return 'subcontract';
  if (/freelance|independant|independent|mission|non salarie/.test(normalized)) return 'freelance';

  return 'other';
}

export function matchesProfileContracts(job: Pick<Job, 'contractType'>, profile: Pick<Profile, 'acceptedContracts'>): boolean {
